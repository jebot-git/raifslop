...
func membership(key:String)->Dictionary:
	for id in games:
		if games[id].members.has(key):return {"course":id,"game":games[id],"player":games[id].members[key]}
	return {}
func command(key:String,name:String,action:String,data:Dictionary,now:float)->bool:
	...
	if action=="join":
		...
		if not m.is_empty():
			if not m.player.retired and not m.game.finished:return m.course==course
			m.game.members.erase(key);m.game.order.erase(key)
		if not games.has(course) or games[course].finished:
			serial+=1;games[course]={"id":"%s/%d"%[run_id,serial],"hole":0,"turn":"","epoch":0,"deadline":0.0,"flight":false,"finished":false,"members":{},"order":[]}
		...
		g.members[key]={"name":name,"present":false,"scores":scores,"strokes":0,"done":false,"retired":false}
		g.order.append(key)
		if g.turn.is_empty():_next(g,now)
		return true
	...
	if action=="presence":
		...
		p.present=present
		if g.turn==key:g.deadline=0.0 if present else now+RETURN_SECONDS
		return true
	if g.turn!=key or int(data.get("epoch",-1))!=g.epoch or not p.present:return false
	if action=="shot":
		if g.flight or p.done or p.strokes>=100:return false
	...
	if action=="settled":
		...
		if data.get("holed",false)==true or p.strokes>=100:
			p.done=true;p.scores.append(p.strokes)
		_next(g,now);return true
	if action=="penalty" and not g.flight and p.strokes>0:
		p.strokes=mini(100,p.strokes+1);return true
	return false
func tick(now:float)->bool:
	...
	for course in games:
		...
		p.done=true;p.scores.append(-1);g.flight=false
		forfeited.emit(key,course,g.hole)
...
func _next(g:Dictionary,now:float)->void:
	...
	for offset in range(1,g.order.size()+1):
		...
		g.deadline=0.0 if g.members[candidate].present else now+RETURN_SECONDS
		return
...
func view(key:String)->Dictionary:
	...
	for member in g.order:
		...
		roster.append({"name":row.name,"scores":row.scores.duplicate(),"retired":row.retired,"present":row.present})
	return {"course":m.course,"id":g.id,"hole":g.hole,"epoch":g.epoch,"your_turn":g.turn==key,"turn_name":g.members[g.turn].name if not g.turn.is_empty() else "","deadline":g.deadline,"present":p.present,"retired":p.retired,"finished":g.finished,"done":p.done,"strokes":p.strokes,"scores":p.scores.duplicate(),"roster":roster,"flight":g.flight}
