...
func toggle()->void:
	if is_instance_valid(game.host_activity) and game.host_activity.clubhouse_round!=null:return
...
func press_buttons(point:Vector3)->void:
	...
	for i in 2:
		...
		if continuous and before.z>.010 and p.z<=.010 and not button_down[i]:
			...
			if absf(contact.x)<.032 and absf(contact.y)<.025:button_down[i]=true;page(-1 if i==0 else 1)
...
func touch_position()->Variant:
	...
	return point
func update()->void:
	if is_instance_valid(game.host_activity) and game.host_activity.clubhouse_round!=null:dock();hide();grip_was_down=true;return
