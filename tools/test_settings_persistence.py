    ...
    with tempfile.TemporaryDirectory(prefix='fishing-settings-') as data:
        ...
        for stage in ('write', 'read'):
            ...
            with log.open('w') as stream:
                result = subprocess.run(cmd + (['--xr-test'] if '--script' in cmd else []), cwd=ROOT, env=env, stdout=stream, stderr=subprocess.STDOUT, timeout=90)
