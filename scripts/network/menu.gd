...
func setup(owner_session: Node, back: Callable) -> void:
	...
	name_input.text_changed.connect(func(value: String): session.display_name=value)
	var row:=HBoxContainer.new(); add_child(row)
	var address:=LineEdit.new(); address.text="127.0.0.1"; address.placeholder_text="Host IP or hostname"; address.size_flags_horizontal=SIZE_EXPAND_FILL; row.add_child(address)
	var port:=SpinBox.new(); port.min_value=1024; port.max_value=65535; port.value=24567; row.add_child(port)
