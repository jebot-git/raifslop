...
func setup(root:Node3D) -> void:
 ...
 fade_mesh=MeshInstance3D.new();var quad:=QuadMesh.new();quad.size=Vector2(4,4);fade_mesh.mesh=quad
...
func _build_menu() -> void:
 ...
 var title:=Label.new();title.text="A little time ashore";title.add_theme_font_size_override("font_size",28);page.add_child(title)
 var copy:=Label.new();copy.text="Tongs: select food to grill → turn over → serve. Watch it brown.\nVR: hold grip to take · trigger to use · release grip to return.\nFood: take a serving, then trigger by your mouth to eat.\nDrinks: trigger to open, then bring to your mouth to sip.\n\nDesktop: H visits · click takes / uses · right-click returns a prop.\nWith tongs: C (desktop) or A (VR) moves food to a cooler spot.\nFood replenishes freely. Cooking does not affect fishing.";copy.add_theme_font_size_override("font_size",19);copy.autowrap_mode=TextServer.AUTOWRAP_WORD_SMART;page.add_child(copy)
...
func _build_station() -> void:
 ...
 var shape:=CollisionShape3D.new();var box:=BoxShape3D.new();box.size=Vector3(2.45,.88,.92);shape.shape=box;shape.position=Vector3(0,.44,0);body.add_child(shape)
 g.motor.collision_mask|=16
 var placard:=Label3D.new();placard.text="SHORE BBQ\nTake a moment. Share a bite.";placard.font_size=42;placard.pixel_size=.0018;placard.position=Vector3(0,1.27,-.38);placard.modulate=Color("f3e7cf");station.add_child(placard)
 hint=Label3D.new();hint.font_size=32;hint.pixel_size=.0015;hint.position=Vector3(0,1.10,.55);hint.billboard=BaseMaterial3D.BILLBOARD_ENABLED;station.add_child(hint)
 for item in service.model.stations[location].items:
  var node:=Node3D.new();node.add_child(load("res://assets/models/bbq/"+item.kind+".glb").instantiate());node.name="BBQItem_"+str(item.id);station.add_child(node);item_nodes[item.id]=node
  var mats:Array=[]
   ...
   if item.kind in Model.FOODS:
    for i in mesh.mesh.get_surface_count():
     ...
     if source is StandardMaterial3D:
      var copy:StandardMaterial3D=source.duplicate();mesh.set_surface_override_material(i,copy);mats.append([copy,copy.albedo_color])
  food_materials[item.id]=mats
 sound=AudioStreamPlayer3D.new();sound.stream=load("res://assets/audio/bbq_sizzle.wav").duplicate();sound.stream.loop_mode=AudioStreamWAV.LOOP_FORWARD;sound.stream.loop_end=88200;sound.volume_db=-28;sound.max_distance=8;sound.unit_size=1;station.add_child(sound);sound.position=Vector3(0,1,0)
...
func hand_pose(hand:int,peer:=-1) -> Transform3D:
 if peer<0 or peer==service.local_id():
  if g.xr:return g.left.global_transform if hand==0 else g.right.global_transform
...
func nearest(at:Vector3,radius:float,food_only:=false) -> int:
 ...
 for item in service.model.stations[location].items:
  if item.owner!=0 or item.place=="gone" or (food_only and item.kind not in Model.FOODS):continue
...
func handle_input(event:InputEvent) -> bool:
 if g.xr or g.menu_open or transitioning:return false
 ...
 if event is InputEventMouseButton and event.pressed:
  ...
  if event.button_index==MOUSE_BUTTON_LEFT:
   ...
   return true
 if event is InputEventKey and event.pressed and not event.echo and event.keycode==KEY_C:use(1,aimed(true),true);return true
 return false
...
func _process(_delta:float) -> void:
 ...
 for item in state.items:
  var node:Node3D=item_nodes[item.id];node.visible=item.place!="gone"
  if item.owner!=0:
  ...
  if item.kind in Model.FOODS:
   ...
   food.position.y=sin(food.rotation.z*.5)*.075
   var cooked:float=maxf(item.cook[0],item.cook[1]);var tint:=Color("9d5422") if cooked<1.85 else Color("30241e")
   for pair in food_materials[item.id]:pair[0].albedo_color=pair[1].lerp(tint,clampf(cooked*.45,0,.95))
 ...
 if not allowed:
  ...
  grip_down=[true,true];trigger_down=[true,true];hint.visible=false;return
 ...
 if g.xr:
  for hand in 2:
   ...
   var at:Vector3=controller.global_position-controller.global_basis.z*(.22 if id in [6,7] else 0)
   ...
   if trigger and not trigger_down[hand] and id>=0:use(hand,target)
   grip_down[hand]=grip;trigger_down[hand]=trigger
 hint.visible=g.head.global_position.distance_to(station.global_position)<3.5
 hint.text="Grip tongs · trigger food: grill → flip → serve" if g.xr else "Click tongs, then food: grill → flip → serve"
 if hovered>=0:
  var item:Dictionary=state.items[hovered]
  if item.kind in Model.FOODS:
   hint.text=item.kind.capitalize()+" · "+(Model.doneness(item) if item.place!="pantry" else "Ready for the grill")
  elif item.kind=="drink":hint.text="Cool drink · take, open and sip"
