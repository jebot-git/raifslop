...
func run():
	...
	var wrist_basis:=Basis.from_euler(Vector3(.3,.7,-.2))
	hand.set_hand_joint_flags(XRHandTracker.HAND_JOINT_WRIST,XRHandTracker.HAND_JOINT_FLAG_ORIENTATION_VALID)
	hand.set_hand_joint_transform(XRHandTracker.HAND_JOINT_WRIST,Transform3D(wrist_basis,Vector3.ZERO))
	var splay:=Basis.from_euler(Vector3(.5,0,.2))
	hand.set_hand_joint_flags(7,XRHandTracker.HAND_JOINT_FLAG_ORIENTATION_VALID)
	hand.set_hand_joint_transform(7,Transform3D(wrist_basis*splay,Vector3.ZERO))
	var rotations:=Hands.finger_rotations(hand)
	check(rotations.IndexProximal.is_equal_approx(splay),"Native finger rotation preserves splay independently of wrist rotation")
	var valid:=preload("res://scripts/tracking/poses.gd").validate_body({"left_finger_rotations":rotations})
	check(not valid.is_empty(),"Finger orientations survive validated tracking and network schema")
	check(preload("res://scripts/tracking/poses.gd").validate_body({"left_finger_rotations":{"UnexpectedBone":Basis.IDENTITY}}).is_empty(),"Unknown finger bones rejected")
