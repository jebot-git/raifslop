...
func run() -> void:
	...
	weak.cue = 0; strong.cue = 0; weak.cue_time=S.COUNTER_WINDOW; strong.cue_time=S.COUNTER_WINDOW; weak.gesture(0); strong.gesture(0); weak.tick(.05,0,0); strong.tick(.05,0,0)
	check(strong.stamina > weak.stamina, "Species endurance controls fatigue per counter")
	var base = fight(); var upgraded = fight(0, 3)
	base.cue = 0; upgraded.cue = 0; base.cue_time=S.COUNTER_WINDOW; upgraded.cue_time=S.COUNTER_WINDOW; base.gesture(0); upgraded.gesture(0); base.tick(.05,0,0); upgraded.tick(.05,0,0)
	...
	for i in range(140): base.tick(.02, 2, 1); upgraded.tick(.02, 2, 1)
