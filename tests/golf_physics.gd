...
func run()->void:
	...
	var grip:=Transform3D(Basis.from_euler(Vector3(.6,.4,.3)),Vector3(.45,1.1,.1))
	var fit=preload("res://addons/golfminus/scripts/golf/club_fit.gd").solve(grip,Vector3(0,.02,-.1),Vector3.FORWARD,.86)
	check(not fit.is_empty(),"Club fitting supports natural controller orientations")
	var club_pose:Transform3D=grip*Transform3D(Basis.from_euler(fit.rotation*PI/180.0),Vector3.ZERO)
	var fitted_tip:Vector3=club_pose*(Vector3(.055,-.86,0)*fit.reach)
	check(fitted_tip.distance_to(fit.target)<.001,"Club fitting aligns head behind ball within one millimetre")
	check(club_pose.origin==grip.origin,"Club fitting preserves tracked grip position and player height")
	var fit_script=preload("res://addons/golfminus/scripts/golf/club_fit.gd")
	check(fit_script.address_direction(Vector3.BACK,Vector3.FORWARD)==Vector3.FORWARD,"Club fitting follows address hemisphere when pin is behind player")
	var flipped:Vector3=fit_script.flip_face(fit.rotation,.86)
	var flipped_pose:Transform3D=grip*Transform3D(Basis.from_euler(flipped*PI/180.0),Vector3.ZERO)
	check((flipped_pose*(Vector3(.055,-.86,0)*fit.reach)).distance_to(fitted_tip)<.001,"Face reversal preserves head position within one millimetre")
	check(flipped_pose.basis.z.dot(club_pose.basis.z)<-.999,"Face reversal rotates the face exactly 180 degrees")
