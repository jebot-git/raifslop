...
func run()->void:
	...
	a.join_course("spyglass");g.motor.set_physics_process(false);await process_frame
	...
	a.resume_course();await process_frame
	...
	g.network.leave()
	a.join_course("pebble");a.start_play("solo");await process_frame
	a.leave();await process_frame;a.resume_course();await process_frame
	...
	a.leave();g.ambience.stop();g.queue_free();await process_frame;await create_timer(.3).timeout
