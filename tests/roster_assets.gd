...
func run():
 ...
 if "--capture" in OS.get_cmdline_user_args():
  DirAccess.make_dir_recursive_absolute("res://test-results/roster")
  await gallery()
  for index in range(32,38):
   g.fish_guide.selected=index;g.fish_guide.screen.queue_redraw()
   for i in 3:await process_frame
   await RenderingServer.frame_post_draw
   g.fish_guide.viewport.get_texture().get_image().save_png("res://test-results/roster/guide-%d.png"%index)
  if g.xr:
   ...
   for group in 3:
    ...
    for side in 2:
     var model=load(S.SPECIES[32+group*2+side].model).instantiate();display.add_child(model)
   ...
   g.fish_guide.reparent(g.head);g.fish_guide.transform=Transform3D(Basis.IDENTITY,Vector3(0,0,-.42));g.fish_guide.show();g.fish_guide.held=true;g.fish_guide.selected=34;g.fish_guide.screen.queue_redraw()
   await capture("guide-ide")
   g.head.compositor=null
 peer.queue_free();g.xr=false;g.queue_free();await process_frame;await create_timer(.3).timeout
 print("ROSTER_ASSETS_RESULT ",checks," checks, ",failures);quit(0 if failures.is_empty() else 1)
func capture(label:String):
 ...
 for eye in effect.results.size():
  var im:Image=effect.results[eye];im.convert(Image.FORMAT_RGBA8);im.linear_to_srgb();im.save_png("res://test-results/roster/"+label+"_eye%d.png"%eye)
func gallery():
 var view:=SubViewport.new();view.size=Vector2i(1500,1450);view.own_world_3d=true;view.render_target_update_mode=SubViewport.UPDATE_ALWAYS;root.add_child(view)
 ...
 var camera:=Camera3D.new();camera.projection=Camera3D.PROJECTION_ORTHOGONAL;camera.size=2.65;camera.position=Vector3(0,0,4);view.add_child(camera)
 var models:Array=[]
 for i in 6:
  var species:Dictionary=S.SPECIES[32+i];var model=load(species.model).instantiate();view.add_child(model)
  model.position=Vector3(-.64+(i%2)*1.28,.87-floori(i/2.0)*.85,0);models.append(model)
 ...
 for angle in [0.0,.55]:
  ...
  image.save_png("res://test-results/roster/gallery"+("-oblique" if angle>0 else "")+".png")
 view.queue_free()
