...
func run() -> void:
	...
	for entry in g.Locations.CATALOG:
		...
		if entry.id=="lake_pier":
			check(is_equal_approx(g.water_level,-.85) and is_equal_approx(g.water_surface.position.y,g.water_level),"Pier water lies below quay foundation")
			...
			check(is_equal_approx(g.water_level,-.35) and not g.water_material.get_shader_parameter("protect_panorama_foreground"),"Travelling restores other water presets")
	...
	check(meshes.size()==4 and meshes[0]!=meshes[1] and meshes[1]!=meshes[2] and meshes[2]!=meshes[3],"Bird species have distinct silhouettes")
	check(insect_kinds==["butterfly","none","midge","dragonfly"] and flight_speeds[0]!=flight_speeds[1] and flight_speeds[2]!=flight_speeds[3],"Waters vary insect species and flight speeds")
