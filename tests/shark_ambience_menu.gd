...
func run():
	...
	check(menu.tabs.get_child_count()==7 and menu.pages.has("controls") and menu.pages.has("leaderboard"),"Seven section tabs including Controls, plus header leaderboard")
